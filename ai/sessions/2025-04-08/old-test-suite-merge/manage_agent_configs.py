...
def _find_section(content: str, start_headings: List[str], start_offset: int = 0) -> Optional[Tuple[str, int, int]]:
    ...
    if not heading_match:
        ...
        start_level = found_heading.count('#') # Fallback to counting '#'
    else:
...
def main():
    ...
    parser.add_argument("-c", "--config", type=str, default="ai/cli_config.yaml", help="Path to the YAML configuration file.")
    ...
    parser_add.add_argument("--md-path", type=str, required=True, help="Path to the agent definition Markdown file.")
    ...
    parser_update.add_argument("--md-path", type=str, required=True, help="Path to the agent definition Markdown file.")
    ...
    args = parser.parse_args() # Revert back to parse_args
    ...
    # --- Load Configuration from YAML (using path from args) ---
    config_path = Path(args.config).resolve() # Use resolved path from argument
    ...
    try:
        ...
        if not cli_config or 'target_json_path' not in cli_config or 'markdown_base_dir' not in cli_config:
            logging.error(f"Invalid configuration format in {config_path}. Missing required keys ('target_json_path', 'markdown_base_dir').")
            sys.exit(1)
        # Resolve paths relative to the config file's directory if they are relative
        config_dir = config_path.parent
        target_json_path_str = cli_config['target_json_path']
        markdown_base_dir_str = cli_config['markdown_base_dir']

        target_json_path = Path(target_json_path_str)
        if not target_json_path.is_absolute():
            target_json_path = (config_dir / target_json_path).resolve()

        markdown_base_dir = Path(markdown_base_dir_str)
        if not markdown_base_dir.is_absolute():
            markdown_base_dir = (config_dir / markdown_base_dir).resolve()

        logging.debug(f"Target JSON path loaded and resolved: {target_json_path}")
        logging.debug(f"Markdown base directory loaded and resolved: {markdown_base_dir}")
    except yaml.YAMLError as e:
        logging.error(f"Error parsing configuration file {config_path}: {e}")
        sys.exit(1)
    except KeyError as e:
         logging.error(f"Missing required key {e} in configuration file {config_path}.")
         sys.exit(1)
    except Exception as e:
    ...
    try:
        if args.operation == "add":
            add_config(args.md_path, target_json_path, markdown_base_dir)
        elif args.operation == "update":
            update_config(args.md_path, target_json_path, markdown_base_dir)
