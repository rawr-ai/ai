...
# Removed unused AgentConfig import
...
# --- Core Operations ---
def add_config(
    markdown_path_str: str, target_json_path: Path, markdown_base_dir: Path
):
    """Adds a new agent configuration from a Markdown file."""
    logger.debug(
        f"Entering add_config with markdown_path_str='{markdown_path_str}', target_json_path='{target_json_path}', markdown_base_dir='{markdown_base_dir}'"
    )
    # --- Path Validation ---
    ...
    logger.debug(
        f"Ensuring target directory exists: {target_json_path.parent}"
    )
    ...
    logger.debug(
        f"Parsed config: slug='{new_config.slug}', name='{new_config.name}'"
    )
    ...
    if any(config.slug == new_config.slug for config in configs):
        logger.error(
            f"Agent with slug '{new_config.slug}' already exists in {target_json_path}."
        )
        raise ValueError(
            f"Error: Agent with slug '{new_config.slug}' already exists in {target_json_path}."
        )

    configs.append(new_config)
    logger.debug(
        f"Appending new config for slug '{new_config.slug}'. Total configs will be {len(configs) + 1}."
    )
...
def update_config(
    ...
    logger.debug(
        f"Entering update_config with markdown_path_str='{markdown_path_str}', target_json_path='{target_json_path}', markdown_base_dir='{markdown_base_dir}'"
    )
    logger.info(
        f"Attempting to update configuration from: {markdown_path_str}"
    )
    # --- Path Validation ---
    markdown_path = Path(markdown_path_str).resolve()
    markdown_base_dir_resolved = markdown_base_dir.resolve()
    ...
    logger.debug(
        f"Parsed config for update: slug='{updated_config.slug}', name='{updated_config.name}'"
    )
    logger.debug(f"Loading existing configs from: {target_json_path}")
    configs = load_configs(target_json_path)
    logger.debug(f"Loaded {len(configs)} existing configs.")
    ...
    for i, config in enumerate(configs):
        if config.slug == updated_config.slug:
            ...
            if preserve_groups:
                logger.debug(
                    f"Preserving existing groups for slug '{config.slug}': {config.groups}"
                )
                ...
                if hasattr(config, "groups") and config.groups is not None:
                    ...
                    logger.warning(
                        f"Existing config for slug '{config.slug}' lacked groups attribute or was None. Cannot preserve."
                    )
            # --- End: Added logic ---
            ...
            found = True
            logger.debug(
                f"Found existing config for slug '{updated_config.slug}' at index {i}. Replacing."
            )  # Existing log
            break

    if not found:
        logger.error(
            f"Agent with slug '{updated_config.slug}' not found in {target_json_path} for update."
        )
        raise ValueError(
            f"Error: Agent with slug '{updated_config.slug}' not found in {target_json_path}."
        )

    logger.debug(
        f"Saving {len(configs)} configs (with update) to: {target_json_path}"
    )
    save_configs(target_json_path, configs)
...
def delete_config(slug: str, target_json_path: Path):
    ...
    logger.debug(
        f"Entering delete_config with slug='{slug}', target_json_path='{target_json_path}'"
    )
    ...
    logger.debug(
        f"Filtering configs to remove slug '{slug}'. Initial count: {initial_length}"
    )
    ...
    if len(configs) == initial_length:
        logger.error(
            f"Agent with slug '{slug}' not found in {target_json_path} for deletion."
        )
        raise ValueError(
            f"Error: Agent with slug '{slug}' not found in {target_json_path}."
        )

    logger.debug(
        f"Saving {len(configs)} configs (after deletion) to: {target_json_path}"
    )
    save_configs(target_json_path, configs)
