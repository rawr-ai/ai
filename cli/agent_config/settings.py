...
def load_cli_config() -> Dict[str, Any]:
    ...
    if config_path_str:
        ...
        logger.info(
            f"Using CLI config path from environment variable {CLI_CONFIG_ENV_VAR}: {config_path}"
        )
    else:
    ...
    logger.debug(
        f"Entering load_cli_config, attempting to load from: {config_path}"
    )
    if not config_path.is_file():
        logger.error(f"CLI configuration file not found at {config_path}.")
        raise FileNotFoundError(
            f"CLI configuration file not found: {config_path}"
        )
    try:
        ...
        if not isinstance(config_data, dict):
            logger.error(
                f"Invalid format: CLI config file {config_path} did not parse as a dictionary."
            )
            raise TypeError(
                f"CLI configuration ({config_path}) is not a valid dictionary."
            )
        ...
        logger.error(
            f"Unexpected error loading CLI config from {config_path}: {e}",
            exc_info=True,
        )
        raise RuntimeError(
            f"Failed to load CLI configuration from {config_path}: {e}"
        )
...
def load_configs(json_path: Path) -> List[AgentConfig]:
    ...
    if not json_path.is_file():
        logger.warning(
            f"Target JSON file not found at {json_path}. Returning empty list."
        )
    ...
    try:
        ...
        TypeError
    ) as e:  # Catch potential issues if data is not list of dicts before parse_obj
...
def save_configs(json_path: Path, configs: List[AgentConfig]):
    ...
    logger.debug(
        f"Entering save_configs with json_path='{json_path}' and {len(configs)} configs"
    )
    try:
        ...
        logger.debug(
            "Converting AgentConfig models to dictionaries for JSON serialization."
        )
        ...
        logger.exception(
            f"An unexpected error occurred during saving to {json_path}"
        )
